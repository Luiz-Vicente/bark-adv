<template>
  <div class="bg-custom-gray d-flex flex-column align-items-center w-100">
    <section class="container" v-if="articles?.length">
      <div class="input-group my-3">
        <input
          type="text"
          class="form-control bg-custom-white text-black"
          v-model="searchTerm"
          placeholder="Buscar artigos"
        />
        <button class="btn btn-custom-gold text-white fw-semibold" @click="searchArticles">Buscar</button>
      </div>
      <div
      ...
      >
        ...
        <div class="d-flex align-items-center justify-content-between w-100">
          <h4 class="text-white m-0">{{ truncate(article.title, 35) }}</h4>
          <i
            class="bi bi-trash-fill text-white fs-4 px-2 pt-1 rounded bg-custom-red-light"
